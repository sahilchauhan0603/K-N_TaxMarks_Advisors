...
const TrademarkForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('/api/trademark', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      if (res.data.success) {
        setSuccess('Your Trademark request has been submitted successfully!');
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 bg-gradient-to-br from-purple-50 to-white border-l-4 border-purple-500 rounded-xl p-6 shadow-md">
      <h4 className="text-lg font-bold text-purple-700 mb-4">Trademark Request</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm text-purple-700 mb-1">Type</label>
          <select name="serviceType" value={form.serviceType} onChange={handleChange} className="w-full border border-purple-200 rounded px-3 py-2 focus:ring-2 focus:ring-purple-400">
            <option value="Search">Trademark Search & Registration</option>
            <option value="Documentation">Legal Documentation & Compliance</option>
            <option value="Protection">IP Protection & Dispute Resolution</option>
          </select>
        </div>
        <div>
