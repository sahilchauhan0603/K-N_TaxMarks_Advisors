...
const BusinessAdvisoryForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('/api/business-advisory', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      if (res.data.success) {
        setSuccess('Your Business Advisory request has been submitted successfully!');
        setForm((prev) => ({ ...prev, businessName: '', notes: '', documents: null }));
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 bg-gradient-to-br from-pink-50 to-white border-l-4 border-pink-500 rounded-xl p-6 shadow-md">
      <h4 className="text-lg font-bold text-pink-700 mb-4">Business Advisory Request</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm text-pink-700 mb-1">Type</label>
          <select name="advisoryType" value={form.advisoryType} onChange={handleChange} className="w-full border border-pink-200 rounded px-3 py-2 focus:ring-2 focus:ring-pink-400">
            <option value="Startup">Startup & MSME Registration</option>
            <option value="Incorporation">Company Incorporation</option>
            <option value="Legal">Legal & Financial Advisory</option>
          </select>
        </div>
        <div>
...
export default BusinessAdvisoryForm;
