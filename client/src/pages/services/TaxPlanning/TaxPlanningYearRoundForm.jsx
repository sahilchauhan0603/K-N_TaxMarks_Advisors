...
const TaxPlanningYearRoundForm = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('/api/tax-year-round', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      if (res.data.success) {
        setSuccess('Your Year-round Strategies request has been submitted successfully!');
        setForm((prev) => ({ ...prev, investmentPlans: '', yearGoals: '', notes: '', documents: null }));
    ...
    <div className="w-full h-full bg-gradient-to-br from-blue-50 to-white">
      ...
      <div className="bg-blue-600 text-white p-4 sm:p-6 rounded-t-2xl">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
          <h4 className="text-xl sm:text-2xl font-bold">Year-round Tax Strategies</h4>
      ...
      <div className="p-4 sm:p-6 space-y-4 h-[calc(450px-100px)] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-blue-700">
                Investment Plans <span className="text-red-500">*</span>
              </label>
              <input 
                name="investmentPlans" 
                value={form.investmentPlans} 
                ...
                placeholder="ELSS, PPF, 80C investments, etc."
              />
            </div>
            <div className="space-y-2">
              <label className="block text-sm font-semibold text-blue-700">
              ...
              <input 
                name="yearGoals" 
                value={form.yearGoals} 
                onChange={handleChange} 
                required
                className="w-full border border-blue-200 rounded-lg px-4 py-3 focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all"
                placeholder="Tax saving targets, financial objectives"
              />
            </div>
          </div>
          ...
          <div className="space-y-3">
            <label className="block text-sm font-semibold text-blue-700">
              Upload Financial Documents (Images Only)
            </label>
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-blue-700">
              Strategic Requirements & Preferences
            </label>
            <textarea 
              name="notes" 
              value={form.notes} 
              ...
              placeholder="Describe your year-round tax strategy needs, risk appetite, investment timeline, specific tax-saving requirements..."
            />
          </div>
          ...
          <div className="pt-4">
            <button 
              type="submit" 
              disabled={loading}
              className="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white font-bold py-3 px-6 rounded-lg shadow-md transition-all duration-200 transform hover:scale-[1.02] disabled:scale-100 disabled:cursor-not-allowed"
            >
              {loading ? (
...
export default TaxPlanningYearRoundForm;
