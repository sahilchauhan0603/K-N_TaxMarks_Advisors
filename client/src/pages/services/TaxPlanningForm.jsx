...
const TaxPlanningForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('/api/tax-planning', formData, {
        headers: { 'Content-Type': 'multipart/form-data' },
      });
      if (res.data.success) {
        setSuccess('Your Tax Planning request has been submitted successfully!');
        setForm((prev) => ({ ...prev, annualIncome: '', notes: '', documents: null }));
      } else {
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 bg-gradient-to-br from-blue-50 to-white border-l-4 border-blue-500 rounded-xl p-6 shadow-md">
      <h4 className="text-lg font-bold text-blue-700 mb-4">Tax Planning Request</h4>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select name="planningType" value={form.planningType} onChange={handleChange} className="w-full border border-blue-200 rounded px-3 py-2 focus:ring-2 focus:ring-blue-400">
            <option value="Personal">Personal & Corporate Tax</option>
            <option value="Year-round">Year-round Strategies</option>
            <option value="Compliance">Tax Compliance & Advisory</option>
          </select>
        </div>
        <div>
          <label className="block text-sm text-blue-700 mb-1">Annual Income (₹)</label>
          <input name="annualIncome" value={form.annualIncome} onChange={handleChange} className="w-full border border-blue-200 rounded px-3 py-2 focus:ring-2 focus:ring-blue-400" />
        </div>
      </div>
      <div className="mt-4">
      ...
      <button type="submit" disabled={loading} className="mt-6 w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded shadow transition disabled:opacity-60">
...
export default TaxPlanningForm;
