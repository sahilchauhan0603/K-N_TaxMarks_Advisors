...
const subTabs = [
  {
    key: "personal_corporate",
    label: "Personal & Corporate Tax",
    endpoint: "/api/tax-personal-corporate",
  },
  {
    key: "year_round",
    label: "Year-round Strategies",
    endpoint: "/api/tax-year-round",
  },
  {
    key: "compliance",
    label: "Tax Compliance & Advisory",
    endpoint: "/api/tax-compliance",
  },
];
...
// Year goal options for dropdown
const yearGoalOptions = [
  "Tax Saving",
  "Wealth Creation",
  "Retirement Planning",
  "Education Planning",
...
// Compliance type options for dropdown
...
const AdminTaxPlanning = () => {
  ...
  const filteredData = data
    ...
    .filter((row) => {
      ...
      if (activeTab === "personal_corporate" && row.entityType)
        return row.entityType === filter;
      if (activeTab === "year_round" && row.yearGoals)
        return row.yearGoals === filter;
      if (activeTab === "compliance" && row.complianceType)
        return row.complianceType === filter;
      return true;
    });
  ...
  const formatColHeader = (col) => {
    ...
    if (col === "documents" || col === "documents") return "Documents";
  ...
  const handleViewDocument = (row) => {
    if (row.documentUrl) {
      setImageModal({
        ...
        title: `Tax Planning Document - ${row.userId?.name || "Unknown"}`,
      });
    }
  };
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="text-3xl font-bold mb-2 text-gray-800">
              Tax Planning Requests
            </h2>
            <p className="text-gray-600">
              Manage and review all tax planning service requests
            </p>
          </div>
          <button
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center">
            <div className="relative flex-grow">
              ...
              <input
                ...
                className="pl-10 w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400"
              />
            </div>
            ...
            {activeTab === "personal_corporate" && (
              <div className="w-full md:w-56">
                <select
                  ...
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400"
                >
            ...
            {activeTab === "year_round" && (
              <div className="w-full md:w-56">
                <select
                  ...
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-200 focus:border-blue-400"
                >
                  <option value="">All Year Goals</option>
                  {yearGoalOptions.map((option) => (
                    <option key={option} value={option}>
                      {option}
                    </option>
                  ))}
                      ...
                      {columns[activeTab].map((col) => (
                        <th
                          ...
                          className={`px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider border-r border-gray-300 ${
                            col === "user"
                              ? "w-56"
                              : col === "entityType" || col === "complianceType"
                              ? "w-36"
                              : col === "incomeDetails" ||
                                col === "investmentPlans" ||
                                col === "yearGoals" ||
                                ...
                                {col === "user" ? (
                                  ...
                                  row.documentUrl ? (
                                    <button
                                      ...
                                      className="text-blue-600 hover:text-blue-800 flex cursor-pointer items-center gap-1 truncate"
                                    >
                                      <svg
